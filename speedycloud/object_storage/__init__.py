...
class AbstractProductAPI(object):
    ...
    def create_sign_str(self, **params):
        canonicalized_resource = urllib.urlencode(params["params"])
        http_header_date = str(
            datetime.utcnow().strftime('%a, %d %b %Y %H:%M:%S GMT')
        )
        params["canonicalized_resource"] = canonicalized_resource
        params["http_header_date"] = http_header_date
        if params['canonicalized_amz_headers'] == "" and params['canonicalized_resource'] == "":
            sign_str = "%(http_method)s\n%(content_md5)s\n%(content_type)s\n%(http_header_date)s\n%(url)s" % params
        elif params['canonicalized_amz_headers'] == "" and params['canonicalized_resource'] != "":
            sign_str = "%(http_method)s\n%(content_md5)s\n%(content_type)s\n%(http_header_date)s\n%(canonicalized_resource)s\n%(url)s" % params
        elif params['canonicalized_amz_headers'] != "" and params['canonicalized_resource'] != "":
            sign_str = "%(http_method)s\n%(content_md5)s\n%(content_type)s\n%(http_header_date)s\n%(canonicalized_amz_headers)s\n%(canonicalized_resource)s\n%(url)s" % params
        elif params['canonicalized_amz_headers'] != "" and params['canonicalized_resource'] == "":
            sign_str = "%(http_method)s\n%(content_md5)s\n%(content_type)s\n%(http_header_date)s\n%(canonicalized_amz_headers)s\n%(url)s" % params
        # print sign_str
        return sign_str
